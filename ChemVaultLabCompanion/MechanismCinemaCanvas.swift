...
struct MechanismCinemaCanvas: View {
    ...
    private var cinemaBackground: some View {
        ZStack {
            ...
            Circle()
                .fill(ChemVaultTheme.accent.opacity(glow ? 0.14 : 0.07))
                .frame(width: glow ? 340 : 270, height: glow ? 340 : 270)
                .blur(radius: 64)
                .offset(x: -130, y: -90)
            
            Circle()
                .fill(Color.red.opacity(glow ? 0.09 : 0.04))
                .frame(width: glow ? 280 : 230, height: glow ? 280 : 230)
                .blur(radius: 60)
                .offset(x: 130, y: 100)
...
struct FocusHalo: View {
    ...
    var body: some View {
        Circle()
            .stroke(color.opacity(pulse ? 0.75 : 0.35), lineWidth: 2)
            .blur(radius: 0.6)
            .shadow(color: color.opacity(0.55), radius: pulse ? 16 : 8)
