...
struct AcademicMechanismTheatre: View {
    ...
    private var academicStageBackground: some View {
        ZStack {
            ...
            Circle()
                .fill(ChemVaultTheme.accent.opacity(pulse ? 0.13 : 0.06))
                .frame(width: pulse ? 310 : 250, height: pulse ? 310 : 250)
                .blur(radius: 58)
                .offset(x: -120, y: -80)
            
            Circle()
                .fill(Color.red.opacity(pulse ? 0.08 : 0.035))
                .frame(width: pulse ? 260 : 220, height: pulse ? 260 : 220)
                .blur(radius: 54)
                .offset(x: 120, y: 80)
